...
public class CoachViewControllerTest {
    ...
    public void testShowFormForUpdateNotFound() {
        // Simulăm lipsa antrenorului pentru ID-ul 1
        when(coachRepository.findById(1L)).thenReturn(Optional.empty());

        try {
            // Această linie va arunca RuntimeException-ul din Controller
            mockMvc.perform(get("/coaches/showFormForUpdate").param("coachId", "1"));
        } catch (Exception e) {
            // Verificăm că excepția este cea corectă
            assertTrue(e.getCause() instanceof RuntimeException);
            assertTrue(e.getCause().getMessage().contains("Antrenorul nu a fost găsit"));
        }
        // Liniile din Controller sunt acum marcate ca vizitate (Covered)
    }
